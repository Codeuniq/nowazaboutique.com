...
function load_items() {
	...
	$.ajax({
		...
		success: function (res) {
			...
			// Clear all tabs
			["abaya", "modest_wear", "ladies_formal", "cotton_salwar", "co_ord_set", "prayer_dress", "hijab", "girls"].forEach(cat => {
				const el = document.getElementById(`${cat}_items`);
				if (el) el.innerHTML = "";
			});

			products.forEach((product, index) => {
				const category = product.item_category?.toLowerCase().replace(/[\s-]+/g, "_")
